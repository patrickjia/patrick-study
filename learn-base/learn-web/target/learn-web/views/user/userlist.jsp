...
<ul class="layui-nav layui-nav-tree site-demo-nav">
  ...
  <li class="layui-nav-item layui-nav-itemed">
    ...
    <dl class="layui-nav-child">
      ...
      <dd class="layui-this"><a href="<%=basePath %>user/list">用户管理</a></dd>
       ...
       <div class="layui-main">
		...
		<!-- <fieldset class="layui-elem-field layui-field-title" style="margin-top: 50px;">
		 --> <!--  <legend>查询列表</legend> -->
		  ...
		  <div  >
		  	<form action="${basePath }user/list" method="get" id="searchForm">
		  		<table class="layui-table" lay-even="" style="height: 20px" lay-skin="nob">
		  			<tr>
		  			<td>人员名称：</td>
		  			<td><input type="text" id="personName" name="personName" value="${condition.personName }" class="layui-input"/></td>
		  			<td>人员登录名：</td>
		  			<td><input type="text" id="loginName" name="loginName" value="${condition.loginName }" class="layui-input"/></td>
		  			</tr>
		  			<tr>
		  			<td>人员类型：</td>
		  			<td>
		  				<div class="layui-input-inline">
							<select name="personType" id="personType" class="form-control">
								...
								<option id="ls" value="2">临时</option>
								<option id="qt" value="3">其他</option>
							</select>
						</div>
		  			</td>
		  			<td>部门名称：</td>
		  			<td><input type="text" id ="deptName" name="deptName" value="${condition.deptName }" class="layui-input"/></td>
		  			 <td>
		  			 <button class="layui-btn layui-btn-radius">查询</button>
		  			 <input type="hidden" id ="curPage" name="curPage"/>
					<input type="hidden" id ="compUserFlag" name="compUserFlag" value="0"/>
					<input type="hidden" id ="deptCode" name="deptCode" value="${condition.deptCode }"/>
		  			 </td>
		  			</tr>
		  		</table>
		  	</form>
		...
		<table class="layui-table" lay-even="" lay-skin="row">
		  ...
		  <c:forEach items="${persons }" var="user">
				<tr id="tr_${user.personId }">
					<td ><input type="checkbox" name="personId" lay-skin="primary" value="${user.personId }"></td>
					<td>${user.personName }</td>
					<td>${user.loginName }</td>
					<td>${user.deptCodeName }</td>
					<td>
						<c:choose>
							<c:when test="${user.personType == 1}">正式</c:when>
							<c:when test="${user.personType == 2 }">临时</c:when>
							<c:when test="${user.personType == 3 }">其他</c:when>
						</c:choose>
					</td>

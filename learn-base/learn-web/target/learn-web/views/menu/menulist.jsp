...
<ul class="layui-nav layui-nav-tree site-demo-nav">
  ...
  <li class="layui-nav-item layui-nav-itemed">
    ...
    <dl class="layui-nav-child">
      ...
      <dd class="layui-this"><a href="<%=basePath %>menu/list">菜单管理</a></dd>
    </dl>
  </li>
  <li class="layui-nav-item layui-nav-itemed">
  	<a href="javascript:;">第三方组件管理</a>
       ...
       <div class="layui-main">
		...
		<!-- <fieldset class="layui-elem-field layui-field-title" style="margin-top: 50px;">
		 --> <!--  <legend>查询列表</legend> -->
		  ...
		  <div  >
		  	<form action="${basePath }menu/list" method="get"
					id="searchForm">
		  		<table class="layui-table" lay-even="" style="height: 20px" lay-skin="nob">
		  			<tr>
		  			<td>菜单编码：</td>
		  			<td><input type="text" id="menuCode" name="menuCode" value="${condition.menuCode }" class="layui-input"/></td>
		  			<td>菜单名称：</td>
		  			<td><input type="text" id="menuName" name="menuName" value="${condition.menuName }" class="layui-input"/></td>
		  			</tr>
		  			<tr>
		  			<td>业务系统：</td>
		...
		<table class="layui-table" lay-even="" lay-skin="row">
		  ...
		  <c:forEach items="${menus }" var="menu">
				<tr id="tr_${menu.menuId }">
					<td ><input type="checkbox" name="menuCode" lay-skin="primary" value="${menu.menuCode}"></td>
					<td>${menu.menuCode }</td>
					<td>${menu.menuName  }</td>
					<td>${menu.busSysName  }</td>
					
					<td>${menu.sortorder}</td>
					<%-- <td>${user.positionName }</td> --%>
					<td>
						<c:if test="${menu.isEnabled == 0 }">
                                                                                                                                       启用
                                       </c:if>
                                       <c:if test="${menu.isEnabled == 1 }">
                                                                                                                                        停用
                                       </c:if>
                                      </td>
					<%-- <td>${user.appcode }</td> --%>
		  ...
		  var active = {
		    setTop: function(){
		      ...
		      layer.open({
				  ...
				  fixed: false, //不固定
				  maxmin: true,
				  content: '${basePath}menu/viewmenu?menuCode='+chk_value
				});

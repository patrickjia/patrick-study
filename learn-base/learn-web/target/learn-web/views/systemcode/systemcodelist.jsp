...
<head>
<meta charset="utf-8">
<title>系统编码规则管理--系统编码规则列表展示</title>
...
<ul class="layui-nav layui-nav-tree site-demo-nav">
  <li class="layui-nav-item layui-nav-itemed">
    ...
    <dl class="layui-nav-child">
      ...
      <dd class="layui-this"><a href="<%=basePath %>systemcode/list">编码规则管理</a></dd>
    </dl>
  </li>
  <li class="layui-nav-item layui-nav-itemed">
    <a href="javascript:;">用户权限管理</a>
     ...
     <!--  <div class="layui-tab-item layui-show"> -->
       <div class="layui-main">
		...
		<table class="layui-table" lay-even="" lay-skin="row">
		  ...
		  <tbody>
		  <c:forEach items="${syscodes }" var="sysCodeRule">
				<tr id="tr_${sysCodeRule.id }">
					<td ><input type="checkbox" name="personId" lay-skin="primary" value="${user.personId }"></td>
					<td>${sysCodeRule.id}</td>
					<td>${sysCodeRule.name }</td>
					<td>${sysCodeRule.prefix }</td>
					<td>${sysCodeRule.suffix }</td>
					<td>${sysCodeRule.currentNum }</td>
					<%-- <td>${user.positionName }</td> --%>
					<td>
						<c:if test="${sysCodeRule.isEnabled == 0 }">
                                                                                                                                       启用
                                       </c:if>
                                       <c:if test="${sysCodeRule.isEnabled == 1 }">
                                                                                                                                        停用
                                       </c:if>
                                      </td>
					<%-- <td>${user.appcode }</td> --%>

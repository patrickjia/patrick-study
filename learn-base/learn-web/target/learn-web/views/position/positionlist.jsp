...
<ul class="layui-nav layui-nav-tree site-demo-nav">
  ...
  <li class="layui-nav-item layui-nav-itemed">
    ...
    <dl class="layui-nav-child">
      ...
      <dd class="layui-this"><a href="<%=basePath %>position/list">岗位管理</a></dd>
       ...
       <div class="layui-main">
		...
		<!-- <fieldset class="layui-elem-field layui-field-title" style="margin-top: 50px;">
		 --> <!--  <legend>查询列表</legend> -->
		  ...
		  <div  >
		  	<form action="${basePath }position/list" method="get"
					id="searchForm">
		  		<table class="layui-table" lay-even="" style="height: 20px" lay-skin="nob">
		  			<tr>
		  			<td>岗位编码：</td>
		  			<td><input type="text" id="postCode"	name="postCode" value="${condition.postCode }" class="layui-input"/></td>
		  			<td>岗位名称：</td>
		  			<td><input type="text" id="postName"	name="postName" value="${condition.postName }" class="layui-input"/></td>
		...
		<table class="layui-table" lay-even="" lay-skin="row">
		  ...
		  <tbody>
		  <c:forEach items="${positions }" var="positoin">
				<tr id="tr_${positoin.postId}">
					<td ><input type="checkbox" name="positionId" lay-skin="primary" value="${positoin.postId }"></td>
					<td>${positoin.postCode }</td>
					<td>${positoin.postName }</td>
					<td>${positoin.appcodeName }</td>
					
					<td>${positoin.sortorder}</td>
					<%-- <td>${user.positionName }</td> --%>
					<td>
						<c:if test="${positoin.isEnabled == 0 }">
                                                                                                                                       启用
                                       </c:if>
                                       <c:if test="${positoin.isEnabled == 1 }">
                                                                                                                                        停用
                                       </c:if>
                                      </td>
					<%-- <td>${user.appcode }</td> --%>
		  ...
		  var active = {
		    setTop: function(){
		    	...
		    	if(btnId=='updateBtn'|btnId=='viewBtn'){
				  $('input[name="positionId"]:checked').each(function(){ 
		      ...
		      layer.open({
				  ...
				  fixed: false, //不固定
				  maxmin: true,
				  content: '${basePath}position/viewposition?positionId='+chk_value
				});
		        ...
		        layer.open({
		          ...
		          ,success: function(layero){
		            ...
		            btn.find('.layui-layer-btn0').attr({
		              href: '${basePath}position/viewposition'
		              ,target: '_blank'
		            });
		          }
		        });

...
<ul class="layui-nav layui-nav-tree site-demo-nav">
  <li class="layui-nav-item layui-nav-itemed">
    ...
    <dl class="layui-nav-child">
      ...
      <dd><a href="<%=basePath %>datadictionary/list">数据字典管理</a></dd>
  ...
  <li class="layui-nav-item layui-nav-itemed">
    <a href="javascript:;">用户权限管理</a>
    <dl class="layui-nav-child">
      <dd class="layui-this"><a href="<%=basePath %>department/list">组织架构管理</a></dd>
      <dd ><a href="<%=basePath %>user/list">用户管理</a></dd>
       ...
       <div class="layui-main">
		...
		<!-- <fieldset class="layui-elem-field layui-field-title" style="margin-top: 50px;">
		 --> <!--  <legend>查询列表</legend> -->
		  ...
		  <div  >
		  	<form action="${basePath }department/list" method="get"
					id="searchForm">
		  		<table class="layui-table" lay-even="" style="height: 20px" lay-skin="nob">
		  			<tr>
		  			<td>部门编码：</td>
		  			<td><input type="text" id="deptCode"	name="deptCode" class="layui-input"/></td>
		  			<td>部门名称：</td>
		  			<td><input type="text" id="deptName"    name="deptName" class="layui-input"/></td>
		  			</tr>
		  			<tr>
		  			<td>部门类型：</td>
		  			<td>
				      ...
				      <select name="deptType" id="deptType">
				        	...
				        	<option id="sel_deptType_01" value="01">企业</option>
							<option id="sel_deptType_02" value="02">政府机关</option>
							<option id="sel_deptType_03" value="03">事业单位</option>
							<option id="sel_deptType_04" value="04">其他</option>
				      </select>
				    </div>
		  			</td>
		...
		<table class="layui-table" lay-even="" lay-skin="row">
		  ...
		  <c:forEach items="${departments }" var="department">
				<tr id="tr_${department.deptId }">
					<td ><input type="checkbox" name="deptId" lay-skin="primary" value="${department.deptId }"></td>
					<td>${department.deptCode }</td>
					<td>${department.deptName }</td>
					<td>${department.supperDeptName }</td>
					<td>
						<c:choose>
							<c:when test="${department.deptType =='01'}">企业</c:when>
							<c:when test="${department.deptType =='02'}">政府机关</c:when>
							<c:when test="${department.deptType =='03'}">事业单位</c:when>
					...
					<td>
						<c:if test="${department.isEnabled == 0 }">
                                                                                                                                       启用
                                       </c:if>
                                       <c:if test="${department.isEnabled == 1 }">
                                                                                                                                        停用
                                       </c:if>
                                      </td>
					<%-- <td>${user.appcode }</td> --%>
...
layui.use('form', function(){
		  ...
		  var active = {
		    setTop: function(){
		      ...
		      layer.open({
				  ...
				  fixed: false, //不固定
				  maxmin: true,
				  content: '${basePath}department/viewdepartment?deptId='+chk_value
				});
		        ...
		        layer.open({
		          ...
		          ,success: function(layero){
		            ...
		            btn.find('.layui-layer-btn0').attr({
		              href: '${basePath}department/viewdepartment'
		              ,target: '_blank'
		            });
		          }
		        });
	...
	var total= parseInt("${page.pages}");
	var curpage = parseInt("${condition.curPage}");	
	layui.use(['laypage', 'layer'], function(){
		  ...
		  laypage({
		    ...
		    ,jump: function(obj,first){
		    	 document.getElementById("curPage").value=obj.curr;
		    	if(!first){
